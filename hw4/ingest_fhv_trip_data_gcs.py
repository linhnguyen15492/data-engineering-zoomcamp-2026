...
BUCKET_NAME = "de_source_bucket"
PROJECT_ID = "data-engineering-486915"
...
MONTHS = [f"{i:02d}" for i in range(10, 13)]
YEARS = [2019]
DOWNLOAD_DIR = "./data"
...
def get_download_url(year, month):
...
def upload_to_gcs(file_path, max_retries=3):
    ...
    # blob_name = os.path.basename(file_path).replace("_", "/")
...
if __name__ == "__main__":
    ...
    for year in YEARS:
        with ThreadPoolExecutor(max_workers=4) as executor:
            file_paths = list(
                executor.map(
                    lambda month: download_file(
                        get_download_url(year=year, month=month),
                        os.path.join(
                            DOWNLOAD_DIR,
                            f"fhv_tripdata_{year}-{month}.csv.gz",
                        ),
                    ),
                    MONTHS,
                )
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            file_paths = list(
                executor.map(
                    lambda gz_path: unzip_file(
                        gz_path, gz_path.replace(".csv.gz", ".csv")
                    ),
